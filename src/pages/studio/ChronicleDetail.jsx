...
export default function ChronicleDetail() {
  ...
  useSEO(book ? {
    ...
    structuredData: withBreadcrumbs({
      ...
      isAccessibleForFree: book.chapters.some(chapter => chapter.status === 'published'),
      hasPart: book.chapters.filter(chapter => chapter.status === 'published').map(chapter => ({
        '@type': 'Chapter',
        name: chapter.title,
        position: chapter.num,
        url: `https://softcursesystems.pages.dev/chronicles/${book.id}/chapter/${chapter.num}`,
        inLanguage: 'en',
      ...
      publisher: { '@type': 'Organization', name: 'Softcurse Systems' },
